...
- Do not treat `content-automation/generated-translations/` as localized copy. `npm run content:run` and `npm run content:expand:800` can write English briefs only, and the sitemap build fails if a language-tagged template URL is included. Reviewed translations in `src/content/blog/<lang>/` are the exception and are served. See [docs/SEO.md](docs/SEO.md).
